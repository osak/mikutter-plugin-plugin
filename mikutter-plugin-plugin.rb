...
Plugin.create(:mikutter_plugin_plugin) do
  ...
  defspell(:compose, :mikutter_plugin_plugin) do |world, body: '', **opts|
    Delayer::Deferred.new.next do
      world.run_command_line(body)
    end
  end
  ...
  world_setting(:mikutter_plugin_plugin, 'Mikutter Plugin Plugin') do
    label "Mikutter Plugin PluginをWorldとして追加します。"
    MPP::World.new({})
  end
